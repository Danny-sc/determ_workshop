...
chosen_params <- list(b = 1/(76*365), mu = 1/(76*365), beta1 = 0.214, beta2 = 0.107, beta3 = 0.428, sigma = 1/7, alpha = 1/50, gamma = 1/14, omega = 1/365)
library(hmer)
library(deSolve)
library(ggplot2)
library(reshape2)
...
tinysigma_emulator <- ems_wave1$R200$mult_sigma(0.1)
op <- par(mfrow = c(1, 3))
cd <- comparison_diag(tinysigma_emulator, validation = validation, targets = targets)
ce <- classification_diag(tinysigma_emulator, validation = validation, targets = targets)
se <- standard_errors(tinysigma_emulator, validation = validation, targets = targets)
par(op)
``
...
hugesigma_emulator <- ems_wave1$R200$mult_sigma(10)
op <- par(mfrow = c(1, 3))
cd <- comparison_diag(hugesigma_emulator, validation = validation, targets = targets)
ce <- classification_diag(hugesigma_emulator, validation = validation, targets = targets)
se <- standard_errors(hugesigma_emulator, validation = validation, targets = targets)
par(op)
``
...
``` {r, lshamp, echo = FALSE, out.width = "75%", fig.cap='LH sampling'}
