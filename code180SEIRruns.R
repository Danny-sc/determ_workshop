...
ode_results <- function(parms, end_time = 365*2) {
  ...
  des = function(time, state, parms) {
    with(as.list(c(state, parms)), {
      ...
      return(list(c(dS, dE, dR, dI)))
    })
  }
  yini = c(S = 900, E = 100, R = 0,  I = 0)
...
par(mar = c(3, 3, 3, 3))
do.call(plot, sol)
x <- c(25,40,100,200,300,350)
sdev <- c(5.8,6.9,1.3, 0.1, 1.5,3.4)
avg = c(116,138,26, 0.7,29.6, 68.9)
#sdev <- c(6.3,12.8,27, 22.2, 15.6, 27.5)
#avg = c(125,257,539, 444,371, 549)
arrows(x, avg-3*sdev, x, avg+3*sdev, length=0.05, angle=90, code=3, lwd = 2)
